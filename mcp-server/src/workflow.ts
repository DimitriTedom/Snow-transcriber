...
Use Snow Transcriber to convert TTS voiceover into exact scene counts for Veo3 / AI video prompt generation.

## Prerequisites
1. Whisper engine running: \`npm run engine:up\` (Docker, port 8000)
2. MCP server configured in your agent (stdio)

## Recommended pipeline (CRAVE & CONQUER)
...
- \`mode\`: \`fixed\` for Veo3 clip math
- \`scene_duration\`: \`6\` (seconds per Veo3 clip)
...
### Step 6 — Generate Veo3 prompts
Feed scene blocks + CC_Veo3_Prompt_Writing_Guide into your prompt writer. One directorial prompt per scene.
...
| \`snow_format_veo3_blocks\` | Export CRAVE & CONQUER scene template |
